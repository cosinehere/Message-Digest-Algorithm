...
CMDA_SHA256::CMDA_SHA256() : p_val(c_sha256initvar, 8) {
    p_salt = nullptr;
    p_saltlen = 0;
...
void CMDA_SHA256::transform() {
    ...
    for (size_t j = 16; j < 64; ++j) {
        uint32_t s0 = RROT(word[j - 15], 7) ^ RROT(word[j - 15], 18) ^
                      (word[j - 15] >> 3);
        uint32_t s1 =
            RROT(word[j - 2], 17) ^ RROT(word[j - 2], 19) ^ (word[j - 2] >> 10);
        word[j] = word[j - 16] + s0 + word[j - 7] + s1;
    }

    uint32_t a = p_val.val[0], b = p_val.val[1], c = p_val.val[2],
             d = p_val.val[3], e = p_val.val[4], f = p_val.val[5],
             g = p_val.val[6], h = p_val.val[7];
