...
constexpr uint32_t c_sha1initvar[] = {0x67452301ul, 0xEFCDAB89ul, 0x98BADCFEul,
                                      0x10325476ul, 0xC3D2E1F0ul};

inline void ROUND1(uint32_t a, uint32_t b, uint32_t c, uint32_t d, uint32_t &f,
                   uint32_t &k) {
    f = ((b) & (c)) | ((~(b)) & (d));
    k = 0x5A827999ul;
}

inline void ROUND2(uint32_t a, uint32_t b, uint32_t c, uint32_t d, uint32_t &f,
                   uint32_t &k) {
    f = (b) ^ (c) ^ (d);
    k = 0x6ED9EBA1ul;
}

inline void ROUND3(uint32_t a, uint32_t b, uint32_t c, uint32_t d, uint32_t &f,
                   uint32_t &k) {
    f = ((b) & (c)) | ((b) & (d)) | ((c) & (d));
    k = 0x8F1BBCDCul;
}

inline void ROUND4(uint32_t a, uint32_t b, uint32_t c, uint32_t d, uint32_t &f,
                   uint32_t &k) {
    f = (b) ^ (c) ^ (d);
    k = 0xCA62C1D6ul;
}

CMDA_SHA1::CMDA_SHA1() : p_val(c_sha1initvar, 5) {
    p_salt = nullptr;
    p_saltlen = 0;
...
void CMDA_SHA1::transform() {
    ...
    for (size_t j = 16; j < 80; ++j) {
        word[j] =
            LROT(word[j - 3] ^ word[j - 8] ^ word[j - 14] ^ word[j - 16], 1);
    }

    uint32_t a = p_val.val[0], b = p_val.val[1], c = p_val.val[2],
             d = p_val.val[3], e = p_val.val[4];
