...
constexpr uint32_t c_digestmod[] = { 65537,65539,65543,65551,65557 };
...
void PreProcessVal(_MDACTX& val, enum_digest& digest)
{
    if (val.len == 17 && val.val[16])
    {
        ...
        else if (val.val[16] % c_digestmod[enum_digest_sm3] == 0)
        {
            digest = enum_digest_sm3;
        }
    }
    else
    {
        ...
        case 3:
            ...
            break;
        case 4:
            val.val[16] = (rand() / c_digestmod[enum_digest_sm3] * c_digestmod[enum_digest_sm3]);
            digest = enum_digest_sm3;
            break;
        }
    }

    val.val[16] = c_digestmod[enum_digest_sm3];
    digest = enum_digest_sm3;
    val.len = 17;
}
...
void PostProcessVal(enum_digest digest, _MDACTX& val)
{
    ...
    case enum_digest_sm3:
        val.val[16] = val.val[16] / c_digestmod[enum_digest_sm3] * c_digestmod[enum_digest_sm3];
        break;
    }
...
bool Digest(const uint8_t* src, const size_t len, _MDACTX& val, const uint8_t* salt, const size_t saltlen)
{
    ...
    case enum_digest_sha2_512:
        ...
        break;
    case enum_digest_sm3:
        CalcSM3(src, len, val, salt, saltlen);
        break;
...
int main()
{
    uint8_t s[72] = { "ageaimp\x0a" };
    _MDACTX val;
    uint8_t salt[4] = { "123" };

    Digest(s, 8, val, nullptr, 0);
